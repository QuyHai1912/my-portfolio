  ...
  return (
    <div id="skill" className={styleSkill.skill}>
      ...
      <div className={styleSkill.skillContain}>
        <h2 className={styleSkill.skillTitle} ref={titleText}>
          Skills Developer
        </h2>
        <p className={styleSkill.titleText}>
          The essential skills I use to bring ideas to life through clean and
          creative web design.
        </p>
          ...
          {groupSkills.map((groupSkill, index) => {
            return (
              <li
                ...
                className={`${styleSkill.groupItem} ${
                  groupSkillActive === groupSkill.name
                    ? styleSkill.groupItemActive
                    : ""
                }`}
              >
                {groupSkill.name}
                <div
                  className={
                    groupSkillActive === groupSkill.name ? "navBar" : "hide"
                  }
                ></div>
              </li>
            );
          ...
          {skills.map((skill, index) => {
            return (
              <li
                key={index}
                className={`${
                  groupSkillActive === "All" || groupSkillActive === skill.group
                    ? styleSkill.listSkillItems
                    : "hide"
                } col l-2-4 m-4`}
              >
                <div className={styleSkill.listSkillItem}>
                  <span className={styleSkill.listSkillItemIcons}>
                    <i
                      className={`${skill.icon} ${styleSkill.listSkillItemIcon}`}
                    ></i>
                  </span>
                  <p className={styleSkill.listSkillItemName}>{skill.name}</p>
                  <span className={styleSkill.listSkillItemLevel}>
                    {skill.level}
                  </span>
                </div>
              </li>
