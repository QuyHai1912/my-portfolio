...
function Home() {
  ...
  return (
    <div id="home" className={styleHome.homeContain}>
      <div className={`${styleHome.homeImg} l-6`}>
        {imagesHome.map((image) => {
          return (
            <div
              ...
              className={`${styleHome.homeContainImg} ${
                imgActive === image.id ? styleHome.homeContainImgActive : ""
              }`}
        ...
        {imagesHome.map((image, id) => {
          return (
            <div
              key={id}
              className={`${
                image.id === imgActive ? styleHome.homeTextItem : "hide"
              } ${styleHome.fadeIn}`}
            >
              <h2 className={styleHome.homeTitle}>
                {image.id === imgActive ? image.title : ""}
              </h2>
              <p className={styleHome.homeTextDetail}>
...
function BackgroundHome() {
  return (
    <div className={styleHome.homeBackground}>
      <Intersect />
      <video
        className={styleHome.homeBackgroundVideo}
        autoPlay
        muted
        loop
        playsInline
